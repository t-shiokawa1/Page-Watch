...
自分専用のWebサイト更新監視ツールです。管理画面は <https://t-shiokawa1.github.io/Page-Watch/> で、監視の実行場所を2つから選べます。
...
- サイト単位で複数の監視URLをまとめて追加・削除・一時停止・今すぐ確認
- サイト登録時に同じドメイン内のリンクを自動収集（最大2階層・40ページ）。詳細画面からURLを手動追加することも可能
...
トップページを登録すると、同一ドメインのナビゲーションリンクを最大2階層・40ページまで自動で追加します。例えば `https://fukazawa.icems.kyoto-u.ac.jp/` を登録すると、トップページから辿れる `/whats-new/` や `/research-2/` も同じ「Fukazawa Group」行で監視されます。PDF・画像などのダウンロードファイルと外部サイトは対象外です。

各サイト行を開くと監視URLの一覧が表示され、URLを1件ずつ追加できます。更新があった場合はサイトを1件として通知し、履歴には変更されたURLを併記します。
